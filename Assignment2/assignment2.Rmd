...
We want to test whether H0: row variable and column variable are independent. The p-value is , so we .... the H0.
...
**E)** Contingency tables are for checking independence or to check if distributions are homogeneous, so yes we would say it is wrong to use it for prediction.
