...
**C)**
pending question: can you improve the interval?
...
coef(lasso.model,s=cv.lasso$lambda.min) #beta’s for the best lambda
...
```
```{r}
ggplot(data=data_titanic, aes(x=Sex, fill=Survived)) + geom_bar(position="dodge")
...
describe(titanic,na.rm = TRUE, interp=FALSE,skew = TRUE, ranges = TRUE,trim=.1,
